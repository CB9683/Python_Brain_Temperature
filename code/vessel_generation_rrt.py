...
def vessel_generation_rrt(vessel_in, prob_map, domain, no_iterations, weight_factor):
    ...
    for n in range(no_iterations):
        ...
        if t == 0:
            ...
            vessel_out[no_points, 0] = no_points
            vessel_out[no_points, 2:5] = new_point
            vessel_out[no_points, 6] = int(vessel_out[min_vertex, 6])
        else:
            vessel_out[no_points, 0] = no_points
            vessel_out[no_points, 2:5] = vessel_out[min_vertex, 2:5] + t * (vessel_out[int(vessel_out[min_vertex, 6]), 2:5] - vessel_out[min_vertex, 2:5])
            vessel_out[no_points, 6] = min_vertex
